...
import { updateCustomer } from "@/lib/mock-api/customers";
...
export function EditCustomerForm({ customer, open, onOpenChange }: EditCustomerFormProps) {
  ...
  useEffect(() => {
    if (customer && open) {
      ...
      setFirmwareUpgrade(customer.deviceRules?.firmwareUpgrade || "inherit");
      setRcOnly(customer.deviceRules?.rcOnly || "inherit");
      setRrm(customer.deviceRules?.rrm || "inherit");
      setNote(customer.notes?.[0]?.note || "");
      setSourceIPsRaw(customer.sourceIP?.join(", ") || "");
      setError(null);
    }
  }, [customer, open]);
